...
BLOCK_COUNT=0
# 1GB = 1073741824
DATA_SIZE_1G=1073741824
DATA_SIZE_10G=10737418240
METADATA_SIZE=64
...
echo "Read 1GB and Write 10GB simultaneously STARTED"
./nvme_reader.sh "$START_BLOCK" "$BLOCK_COUNT" "$METADATA_SIZE" "$DATA_SIZE_1G" "$READ_OUTPUT_FILE" &
./nvme_writer.sh "$START_BLOCK" "$BLOCK_COUNT" "$METADATA_SIZE" "$DATA_SIZE_10G" "$WRITE_OUTPUT_FILE" 
echo "Read and write Completed"
