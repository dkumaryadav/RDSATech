...
echo "Read 10GB and Write 1GB simultaneously STARTED"
./nvme_reader.sh "$START_BLOCK" "$BLOCK_COUNT" "$METADATA_SIZE" "$DATA_SIZE_10G" "$READ_OUTPUT_FILE" &
./nvme_writer.sh "$START_BLOCK" "$BLOCK_COUNT" "$METADATA_SIZE" "$DATA_SIZE_1G" "$WRITE_OUTPUT_FILE" 
